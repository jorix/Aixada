...
class abstract_import_manager {
	...
	/**
	 * Indicates if import is using a db transaction or not.
	 * @var bool
	 */
    ...
    public function import($append_new=false, $keep_match_field = false, $reset = false){
        try {
    	    ...
    	    if ($append_new && count($insert_rows)){
                $imported_rows_count += $this->insert_rows($insert_rows, $keep_match_field);
            }
    ...
    /**
     * If import is in transaction mode, then rollbacks the current transaction.
     */
    protected function import_rollback() {
        if ($this->_use_transaction) {
            DBWrap::get_instance()->rollback();
        }
    }

    /**
     * If import is in transaction mode, then commits the current transaction.
     */
    protected function import_commit() {
        if ($this->_use_transaction) {
            DBWrap::get_instance()->commit();
        }
    }
